...
# MAGIC %run ../code/hl7_sb/encntr_nte_hist
...
def send_a_list_of_messages_to_topic(topic_conn_str, topic_name, payloads):        
    ...
    servicebus_client_topic_send = ServiceBusClient.from_connection_string(topic_conn_str, logging_enable=True, uamqp_transport=True)
...
def invoke_all(messages):
    ...
    df_messages = spark.createDataFrame(messages, StructType([StructField("msg", StringType()), StructField("msg_nm", IntegerType()), StructField("msg_enqueued_tsp", TimestampType())])).withColumn("insert_user_id", lit(insert_user_id))
    
    df_hist = df_messages.withColumn("adt_hist", invoke_adt_hist_process("msg", "insert_user_id", "msg_nm", "msg_enqueued_tsp")) \
      .withColumn("encntr_dx_hist",invoke_encntr_dx_process("msg","insert_user_id", "msg_enqueued_tsp")) \
      .withColumn("encntr_er_complnt_hist",invoke_encntr_er_complaint_process("msg","insert_user_id", "msg_enqueued_tsp")) \
      .withColumn("encntr_visit_rsn_hist",invoke_encntr_visit_rsn_process("msg","insert_user_id", "msg_enqueued_tsp")) \
      .withColumn("encntr_nte_hist",invoke_encntr_nte_process("msg","insert_user_id", "msg_enqueued_tsp")) \
      .select("adt_hist","encntr_dx_hist", "encntr_er_complnt_hist","encntr_visit_rsn_hist", "encntr_nte_hist").toJSON()
...
def queue_to_topic(queue_name, queue_conn_str, topic_name, topic_conn_str):
  ...
  servicebus_client_receive = ServiceBusClient.from_connection_string(queue_conn_str, logging_enable=True, uamqp_transport=True)
  messages = []
  batch_size = 250
  ...
  with servicebus_client_receive:
      ...
      with receiver:
          for msg in receiver:
              messages.append((str(msg), msg.sequence_number, msg.enqueued_time_utc))
              receiver.complete_message(msg)
              if len(messages) == batch_size:
                  payloads = invoke_all(messages)
                  messages = []
                  send_a_list_of_messages_to_topic(topic_conn_str, topic_name, payloads)
                  print(f'Sent 250 messages to topic in Time : {time.time() - start_time}')
                  start_time = time.time()
