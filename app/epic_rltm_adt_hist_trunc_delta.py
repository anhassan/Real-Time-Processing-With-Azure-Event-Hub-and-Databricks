...
def get_truncated_records(table_name,num_retention_days):
    ...
    df_expired_csn = df.where(col("hosp_disch_tm").isNotNull()) \
                       .where( to_date(col("hosp_disch_tm")) < date_sub(to_date(current_timestamp()),num_retention_days) ) \
...
child_tables = ["encntr_dx_hist","encntr_er_complnt_hist","encntr_visit_rsn_hist","encntr_nte_hist","flwsht"]
z_order_cols = 6*["row_insert_tsp"]
retention_period = 2
