...
hl7_sample = """MSH|^~\&|EPIC|SJM|||20220602160742|39248|ADT^A03^ADT_A03|563782752|P|2.5.1
...
def test_get_hosp_disch_time(hl7_sample_parsed):

    returned = get_hosp_disch_time(hl7_sample_parsed)
    expected = "2022-06-02 16:07:00-05:00"
...
hl7_sample_parsed = parse_msg(hl7_sample)
test_get_hosp_disch_time(hl7_sample_parsed)

# COMMAND ----------

def test_get_acuity_level(hl7_sample_parsed):
  
  returned = get_acuity_level(hl7_sample_parsed)
  expected = "Emergent"
  if(returned == expected) :
    return "Test Case passed Successfully"
  else :
...
hl7_sample_parsed = parse_msg(hl7_sample)
test_get_acuity_level(hl7_sample_parsed)

# COMMAND ----------

def test_get_adt_arrival_time(hl7_sample_parsed):

    returned = get_adt_arrival_time(hl7_sample_parsed)
    expected = None
    if returned == expected :
      return "Test Case passed Successfully"
    else :
...
hl7_sample_parsed = parse_msg(hl7_sample)
test_get_adt_arrival_time(hl7_sample_parsed)

# COMMAND ----------

def test_get_hosp_admsn_time(hl7_sample_parsed):

    returned = get_hosp_admsn_time(hl7_sample_parsed)
    expected = None
    if returned == expected :
      return "Test Case passed Successfully"
    else :
...
hl7_sample_parsed = parse_msg(hl7_sample)
test_get_hosp_admsn_time(hl7_sample_parsed)

# COMMAND ----------

def test_invoke_adt_hist_process(msg, insert_user_id, msg_nm):

    df = spark.createDataFrame(
        [[hl7_sample, insert_user_id, msg_nm]],
        StructType(
            [
                StructField("msg", StringType()),
                StructField("insert_user_id", StringType()),
                StructField("msg_nm", StringType()),
            ]
        ),
    ).withColumn(
        "returned",
        invoke_adt_hist_process(col("msg"), col("insert_user_id"), col("msg_nm")),
    )
    ...
    expected = str(
        {
            ...
            "msg_nm": "1",
            "bod_id": "563782752",
            "pat_mrn_id": "E1404972671",
            "pat_enc_csn_id": 248762958,
            "birth_date": "1950-08-20 00:00:00-05:00",
            "death_date": None,
            "pat_class_abbr": "OBSERVATION",
            "hosp_admsn_time": None,
            "hosp_disch_time": "2022-06-02 16:07:00-05:00",
            "department_abbr": "SJMED",
            "loc_abbr": "SJM",
            "room_nm": "CHECKOUT",
            "bed_label": "CHECKOUT",
            "bed_status": "Ready",
            "sex_abbr": "M",
            "means_of_arrv_abbr": "Car",
            "acuity_level_abbr": "Emergent",
            "ed_disposition_abbr": "AMA",
            "disch_disp_abbr": "HOME",
            "adt_arrival_time": None,
            "hsp_account_id": 61001224159,
            "accommodation_abbr": None,
            "user_id": "39248",
...
test_invoke_adt_hist_process(hl7_sample, insert_user_id, msg_nm)
